...
use constants;
use crypto::{EncType, PrivateKey, PublicKey, SessionKey, Signature, SigningPrivateKey,
             SigningPublicKey, SigType};
...
pub fn gen_sig_type<'a>(input: (&'a mut [u8], usize),
                        sig_type: &SigType)
...
pub fn gen_enc_type<'a>(input: (&'a mut [u8], usize),
                        enc_type: &EncType)
                        -> Result<(&'a mut [u8], usize), GenError> {
...
pub fn gen_session_key<'a>(input: (&'a mut [u8], usize),
                           k: &SessionKey)
                           -> Result<(&'a mut [u8], usize), GenError> {
    gen_slice!(input, k.0)
}
...
pub fn gen_public_key<'a>(input: (&'a mut [u8], usize),
                          key: &PublicKey)
                          -> Result<(&'a mut [u8], usize), GenError> {
...
pub fn gen_private_key<'a>(input: (&'a mut [u8], usize),
                           key: &PrivateKey)
                           -> Result<(&'a mut [u8], usize), GenError> {
    gen_slice!(input, key.0)
}
...
pub fn gen_signing_key<'a>(input: (&'a mut [u8], usize),
...
pub fn signing_private_key<'a>(input: &'a [u8],
                               sig_type: SigType)
...
pub fn gen_signing_private_key<'a>(input: (&'a mut [u8], usize),
...
pub fn signature<'a>(input: &'a [u8], sig_type: &SigType) -> IResult<&'a [u8], Signature> {
    do_parse!(input, sig: take!(sig_type.sig_len()) >> (Signature::from_bytes(sig_type, sig)))
}

pub fn gen_signature<'a>(input: (&'a mut [u8], usize),
                         sig: &Signature)
                         -> Result<(&'a mut [u8], usize), GenError> {
